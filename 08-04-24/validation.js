$(document).ready(function() {
  ...
  $('.enquiry-form input[name=sname], .enquiry-form input[name=email], .enquiry-form input[name=city], .enquiry-form input[name=state], .enquiry-form input[name=bname]').on('input', function(e) {
    var value = $(this).val();
    if (value.length > 35) {
      $(this).val(value.slice(2, 35));
    }
  });
  ...
  $('.enquiry-form').submit(function(e) {
    ...
    var error = false;
    var nameRegex = /^(?!\s*$)[a-zA-Z ]+$/; // Updated regex to disallow only spaces without alphabets and not allow spaces at the start
    var phoneRegex = /^[6789][0-9]{9}$/; // Updated regex to enforce exactly 10 digits for phone number
    var emailRegex = /^[a-z0-9]+(?:[._%+-]?[a-z0-9]+)*@[a-z0-9.-]+\.[a-z]{2,4}$/;
    ...
    var state = $('.enquiry-form input[name=state]');
    ...
    if (phone.val() == '') {
      ...
      phone.after('<div class="error">Please enter a valid 10-digit phone number.</div>');
      phone.css('border-color', 'red');
      error = true;
    } else {
