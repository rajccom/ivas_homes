...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
        ...
        foreach ($requiredFields as $fieldName => $fieldLabel) {
            if (empty($_POST[$fieldName])) {
                ...
                if ($fieldName === 'sname' && !ctype_alpha(str_replace(' ', '', $_POST[$fieldName]))) {
                    ...
                    exit();
                } elseif ($fieldName === 'email' && !filter_var($_POST[$fieldName], FILTER_VALIDATE_EMAIL)) {
    ...
    //$conn = mysqli_connect("localhost", "root", "", "ivas_homes"); 
    ...
    $msg = strip_tags(addslashes($_POST['message']));
    ...
    // $stmt = mysqli_prepare($conn, "INSERT INTO contact_lead (category, sname, phone, email, city, msg) VALUES (?, ?, ?, ?, ?, ?)");
    // mysqli_stmt_bind_param($stmt, "ssssss", $category, $name, $phone, $emails, $city, $msg);
    ...
    // mysqli_stmt_close($stmt);
        ...
        // Build the SQL query
        $sql = "INSERT INTO landing_pages_lead (category, sname, phone, email, city, msg, utm_source, utm_medium, utm_campaign, utm_term) VALUES ('$category', '$name', '$phone', '$emails', '$city', '$msg', '$utm_source', '$utm_medium', '$utm_campaign', '$utm_term')";
        // Execute the query
